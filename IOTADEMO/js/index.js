// IOTA full node : https://hackmd.io/s/BkbaBvpHG

// Jquery: when the web is loaded and ready, perform the javascript
$(document).ready(function() {

    //  Instantiate IOTA with google cloud 'http://google_cloud_IP:14265'
    var iota = new IOTA({
        'host': 'http://104.196.202.51',
        'port': 14265
    });
            ...
            if (accountData.transfers.length > checkedTxs) {
                ...
                accountData.transfers.forEach(function(transfer) {
                    ...
                    try {
                        ...
                        ncredit = ncredit + parseInt(message.credit);
                        var newTx = {
                            'name': message.name,
                            'message': message.message,
                            'value': transfer[0].value,
                            'credit': String(ncredit),
                            'time': message.time
                        }
    ...
    $("#seedSubmit").on("click", function() {
        ...
        getAccountInfo();
        setInterval(getAccountInfo, 10000);
    });
